...
describe("auth setup helpers", () => {
  ...
  test("uses a hard navigation after setup so the session cookie is reloaded", () => {
    const location = { assign: rs.fn() };

    navigateAfterSetup(location);

    expect(location.assign).toHaveBeenCalledWith("/workspace");
  });
});
