...
当前版本已完成研究闭环和来源分级的代码实现与离线回归，仍需配置真实服务完成持续质量及生产环境验收。历史 B站播放器案例未完成完整审计，不能作为端到端研究成功的证明。详情见 [验收记录](docs/COMPETITIVE_RESEARCH_CONFIDENCE_AND_WORKSPACE_ZH.md)。
...
Gateway 启动时运行独立的调查数据库迁移，当前链为 `ci_0011`：
...
`preflight` 检查配置是否存在，不调用模型或搜索，也不代表服务连通性验收。完整后端测试入口为 `make test` 和 `make test-blocking-io`，适用环境及本地已知限制见 [验证记录](docs/COMPETITIVE_RESEARCH_CONFIDENCE_AND_WORKSPACE_ZH.md)。
